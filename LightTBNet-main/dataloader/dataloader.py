...
class MCSZDataset(Dataset): 
    """
    not actually Montgomery (MC) & Shenzen (SZ) Dataset class but the class is named MCSZDataset bcoz i didnt want to change it in very single file  
    ...
    def __getitem__(self, idx):
        ...
        transforms_list = [
        A.CLAHE(clip_limit=(0.01*max_value,0.01*max_value), p=1) if self.apply_clahe else None,
        A.HorizontalFlip(p=0.5) if self.do_transform else None,
        A.ShiftScaleRotate(shift_limit=0.1, rotate_limit=15, border_mode=cv2.BORDER_CONSTANT, value=0, p=0.5) if self.do_transform else None,
        A.Resize(height=self.configs['img_dim_ai'],width=self.configs['img_dim_ai']),
        A.Normalize(mean=0.5,std=0.5),
        ToTensorV2(),
        ]
        transform = A.Compose([t for t in transforms_list if t is not None])
